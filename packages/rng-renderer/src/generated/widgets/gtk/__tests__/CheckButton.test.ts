import { CheckButton } from "@/generated/widgets.ts"
...
describe("CheckButton", () => {
  ...
  test("should connect onNotifyGroup", () => {
    ...
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::group", expect.any(Function))
  })
  ...
  test("should connect onNotifyInconsistent", () => {
    ...
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::inconsistent", expect.any(Function))
  })
