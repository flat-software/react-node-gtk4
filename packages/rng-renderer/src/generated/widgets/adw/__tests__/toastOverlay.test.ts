...
import {ToastOverlay} from "@/generated/widgets.ts";

describe("ToastOverlay", () => {
  let widget: ToastOverlay;

  beforeEach(() => {
    widget = new ToastOverlay({}, ToastOverlay.createNode());
  });
  ...
  test("should append child", () => {
    const child = new ToastOverlay({}, ToastOverlay.createNode());
  ...
  test("should insert child before", () => {
    const child = new ToastOverlay({}, ToastOverlay.createNode());
    const sibling = new ToastOverlay({}, ToastOverlay.createNode());
    widget.appendChild(child);
