import { ColorButton } from "@/generated/widgets.ts"
...
describe("ColorButton", () => {
  ...
  test("should connect onColorSet", () => {
    ...
    expect(handler).toBeDefined()
    handler()
    expect(widget.node.on).toHaveBeenCalledWith("color-set", expect.any(Function))
    expect(callback).toHaveBeenCalled()
  })
  ...
  test("should connect onColorActivated", () => {
    ...
    expect(handler).toBeDefined()
    handler()
    expect(widget.node.on).toHaveBeenCalledWith("color-activated", expect.any(Function))
    expect(callback).toHaveBeenCalled()
  })
  ...
  test("should connect onNotifyShowEditor", () => {
    ...
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::showEditor", expect.any(Function))
  })
  ...
  test("should connect onNotifyRgba", () => {
    ...
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::rgba", expect.any(Function))
  })
  ...
  test("should connect onNotifyUseAlpha", () => {
    ...
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::useAlpha", expect.any(Function))
  })
