...
describe("ActionRow", () => {
  ...
  test("should connect onNotifyActivatableWidget", () => {
    ...
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::activatableWidget", expect.any(Function))
  })
  ...
  test("should connect onNotifySubtitleSelectable", () => {
    ...
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::subtitleSelectable", expect.any(Function))
  })
