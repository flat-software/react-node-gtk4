import { AspectFrame } from "@/generated/widgets.ts"
...
describe("AspectFrame", () => {
  ...
  test("should connect onNotifyObeyChild", () => {
    ...
    const handler = widget.handlers["notify::obeyChild"]
    expect(handler).toBeDefined()
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::obeyChild", expect.any(Function))
  })
  ...
  test("should connect onNotifyRatio", () => {
    ...
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::ratio", expect.any(Function))
  })
