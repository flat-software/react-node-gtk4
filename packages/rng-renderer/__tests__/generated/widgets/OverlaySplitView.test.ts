import { OverlaySplitView } from "@/generated/widgets.ts"
import Gtk from "@/generated/girs/node-gtk-4.0.js"
import Adw from "@/generated/girs/node-adw-1.js"

describe("OverlaySplitView", () => {
  let widget: OverlaySplitView

  beforeEach(() => {
    widget = new OverlaySplitView({}, OverlaySplitView.createNode(    ))
  })
  ...
  test("should connect onNotifyEnableHideGesture", () => {
    ...
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::enableHideGesture", expect.any(Function))
  })
  ...
  test("should connect onNotifyEnableShowGesture", () => {
    ...
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::enableShowGesture", expect.any(Function))
  })
  ...
  test("should connect onNotifyPinSidebar", () => {
    ...
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::pinSidebar", expect.any(Function))
  })
  ...
  test("should connect onNotifyShowSidebar", () => {
    ...
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::showSidebar", expect.any(Function))
  })
