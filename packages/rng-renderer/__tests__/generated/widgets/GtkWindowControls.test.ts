import { WindowControls } from "@/generated/widgets.ts"
...
describe("WindowControls", () => {
  ...
  test("should connect onNotifyEmpty", () => {
    ...
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::empty", expect.any(Function))
  })
  ...
  test("should connect onNotifySide", () => {
    ...
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::side", expect.any(Function))
  })
  ...
  test("should connect onNotifyUseNativeControls", () => {
    ...
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::useNativeControls", expect.any(Function))
  })
