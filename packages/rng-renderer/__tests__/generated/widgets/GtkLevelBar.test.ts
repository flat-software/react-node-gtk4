import { LevelBar } from "@/generated/widgets.ts"
...
describe("LevelBar", () => {
  ...
  test("should connect onOffsetChanged", () => {
    ...
    handler()
    expect(widget.node.on).toHaveBeenCalledWith("offset-changed", expect.any(Function))
  ...
  test("should connect onNotifyMaxValue", () => {
    ...
    const handler = widget.handlers["notify::maxValue"]
    expect(handler).toBeDefined()
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::maxValue", expect.any(Function))
  })
  ...
  test("should connect onNotifyMinValue", () => {
    ...
    const handler = widget.handlers["notify::minValue"]
    expect(handler).toBeDefined()
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::minValue", expect.any(Function))
  })
  ...
  test("should connect onNotifyMode", () => {
    ...
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::mode", expect.any(Function))
  })
