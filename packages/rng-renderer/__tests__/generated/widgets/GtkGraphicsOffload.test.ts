...
describe("GraphicsOffload", () => {
  ...
  test("should set blackBackground", () => {
    const newValue = true
    widget.set("blackBackground", newValue)
    expect(widget.node.setBlackBackground).toHaveBeenCalledWith(newValue)
  })
  ...
  test("should connect onNotifyBlackBackground", () => {
    const callback = jest.fn()

    widget.set("onNotifyBlackBackground", callback)

    const handler = widget.handlers["notify::blackBackground"]
    expect(handler).toBeDefined()
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::blackBackground", expect.any(Function))
  })
  ...
  test("should connect onNotifyEnabled", () => {
    ...
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::enabled", expect.any(Function))
  })
