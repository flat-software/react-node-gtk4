...
describe("NavigationPage", () => {
  ...
  test("should connect onShowing", () => {
    ...
    expect(handler).toBeDefined()
    handler()
    expect(widget.node.on).toHaveBeenCalledWith("showing", expect.any(Function))
    expect(callback).toHaveBeenCalled()
  })
  ...
  test("should connect onNotifyCanPop", () => {
    ...
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::canPop", expect.any(Function))
  })
  ...
  test("should connect onNotifyTag", () => {
    ...
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::tag", expect.any(Function))
  })
