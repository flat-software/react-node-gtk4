...
describe("FlowBox", () => {
  ...
  test("should connect onActivateCursorChild", () => {
    ...
    handler()
    expect(widget.node.on).toHaveBeenCalledWith("activate-cursor-child", expect.any(Function))
  ...
  test("should connect onChildActivated", () => {
    ...
    expect(handler).toBeDefined()
    handler()
    expect(widget.node.on).toHaveBeenCalledWith("child-activated", expect.any(Function))
    expect(callback).toHaveBeenCalled()
  })
  ...
  test("should connect onSelectedChildrenChanged", () => {
    ...
    expect(handler).toBeDefined()
    handler()
    expect(widget.node.on).toHaveBeenCalledWith("selected-children-changed", expect.any(Function))
    expect(callback).toHaveBeenCalled()
  })
  ...
  test("should connect onToggleCursorChild", () => {
    ...
    expect(handler).toBeDefined()
    handler()
    expect(widget.node.on).toHaveBeenCalledWith("toggle-cursor-child", expect.any(Function))
    expect(callback).toHaveBeenCalled()
  })
  ...
  test("should connect onNotifyAcceptUnpairedRelease", () => {
    ...
    const handler = widget.handlers["notify::acceptUnpairedRelease"]
    expect(handler).toBeDefined()
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::acceptUnpairedRelease", expect.any(Function))
  })
  ...
  test("should connect onNotifyMaxChildrenPerLine", () => {
    ...
    const handler = widget.handlers["notify::maxChildrenPerLine"]
    expect(handler).toBeDefined()
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::maxChildrenPerLine", expect.any(Function))
  })
  ...
  test("should connect onNotifyMinChildrenPerLine", () => {
    ...
    const handler = widget.handlers["notify::minChildrenPerLine"]
    expect(handler).toBeDefined()
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::minChildrenPerLine", expect.any(Function))
  })
