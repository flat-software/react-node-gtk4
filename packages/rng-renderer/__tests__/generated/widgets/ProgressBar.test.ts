...
describe("ProgressBar", () => {
  ...
  test("should connect onNotifyFraction", () => {
    ...
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::fraction", expect.any(Function))
  })
  ...
  test("should connect onNotifyPulseStep", () => {
    ...
    const handler = widget.handlers["notify::pulseStep"]
    expect(handler).toBeDefined()
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::pulseStep", expect.any(Function))
  })
  ...
  test("should connect onNotifyShowText", () => {
    ...
    const handler = widget.handlers["notify::showText"]
    expect(handler).toBeDefined()
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::showText", expect.any(Function))
  })
