import { ComboRow } from "@/generated/widgets.ts"
...
describe("ComboRow", () => {
  ...
  beforeEach(() => {
    widget = new ComboRow({}, ComboRow.createNode(    ))
  })
  ...
  test("should set useSubtitle", () => {
    const newValue = true
    widget.set("useSubtitle", newValue)
    expect(widget.node.setUseSubtitle).toHaveBeenCalledWith(newValue)
  })
  ...
  test("should connect onNotifyExpression", () => {
    ...
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::expression", expect.any(Function))
  })
  ...
  test("should connect onNotifyListFactory", () => {
    ...
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::listFactory", expect.any(Function))
  })
  ...
  test("should connect onNotifySearchMatchMode", () => {
    ...
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::searchMatchMode", expect.any(Function))
  })
  ...
  test("should connect onNotifySelected", () => {
    ...
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::selected", expect.any(Function))
  })
  ...
  test("should connect onNotifySelectedItem", () => {
    ...
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::selectedItem", expect.any(Function))
  })

  test("should connect onNotifyUseSubtitle", () => {
    const callback = jest.fn()

    widget.set("onNotifyUseSubtitle", callback)

    const handler = widget.handlers["notify::useSubtitle"]
    expect(handler).toBeDefined()
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::useSubtitle", expect.any(Function))
  })
