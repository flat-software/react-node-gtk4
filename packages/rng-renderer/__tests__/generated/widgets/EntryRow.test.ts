import { EntryRow } from "@/generated/widgets.ts"
...
describe("EntryRow", () => {
  ...
  beforeEach(() => {
    widget = new EntryRow({}, EntryRow.createNode(    ))
  })
  ...
  test("should connect onEntryActivated", () => {
    ...
    expect(handler).toBeDefined()
    handler()
    expect(widget.node.on).toHaveBeenCalledWith("entry-activated", expect.any(Function))
    expect(callback).toHaveBeenCalled()
  })
  ...
  test("should connect onNotifyShowApplyButton", () => {
    ...
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::showApplyButton", expect.any(Function))
  })
