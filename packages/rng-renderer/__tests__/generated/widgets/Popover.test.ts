import { Popover } from "@/generated/widgets.ts"
...
describe("Popover", () => {
  ...
  test("should connect onNotifyCascadePopdown", () => {
    ...
    const handler = widget.handlers["notify::cascadePopdown"]
    expect(handler).toBeDefined()
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::cascadePopdown", expect.any(Function))
  })
  ...
  test("should connect onNotifyHasArrow", () => {
    ...
    const handler = widget.handlers["notify::hasArrow"]
    expect(handler).toBeDefined()
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::hasArrow", expect.any(Function))
  })
  ...
  test("should connect onNotifyPointingTo", () => {
    ...
    const handler = widget.handlers["notify::pointingTo"]
    expect(handler).toBeDefined()
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::pointingTo", expect.any(Function))
  })
