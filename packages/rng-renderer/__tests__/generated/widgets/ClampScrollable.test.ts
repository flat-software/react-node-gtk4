...
describe("ClampScrollable", () => {
  ...
  beforeEach(() => {
    widget = new ClampScrollable({}, ClampScrollable.createNode(    ))
  })

  test("should set maximumSize", () => {
    const newValue = 1
    widget.set("maximumSize", newValue)
    expect(widget.node.setMaximumSize).toHaveBeenCalledWith(newValue)
  })

  test("should set tighteningThreshold", () => {
    const newValue = 1
    widget.set("tighteningThreshold", newValue)
    expect(widget.node.setTighteningThreshold).toHaveBeenCalledWith(newValue)
  })

  test("should set unit", () => {
    const newValue = Adw.LengthUnit.PX
    widget.set("unit", newValue)
    expect(widget.node.setUnit).toHaveBeenCalledWith(newValue)
  })
  ...
  test("should connect onNotifyMaximumSize", () => {
    const callback = jest.fn()

    widget.set("onNotifyMaximumSize", callback)

    const handler = widget.handlers["notify::maximumSize"]
    expect(handler).toBeDefined()
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::maximumSize", expect.any(Function))
  })

  test("should connect onNotifyTighteningThreshold", () => {
    const callback = jest.fn()

    widget.set("onNotifyTighteningThreshold", callback)

    const handler = widget.handlers["notify::tighteningThreshold"]
    expect(handler).toBeDefined()
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::tighteningThreshold", expect.any(Function))
  })

  test("should connect onNotifyUnit", () => {
    const callback = jest.fn()

    widget.set("onNotifyUnit", callback)

    const handler = widget.handlers["notify::unit"]
    expect(handler).toBeDefined()
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::unit", expect.any(Function))
  })
  ...
  test('should append child', () => {
    const child = new ClampScrollable({}, ClampScrollable.createNode())
  ...
  test('should insert child before', () => {
    const child = new ClampScrollable({}, ClampScrollable.createNode())
    const sibling = new ClampScrollable({}, ClampScrollable.createNode())
    widget.appendChild(child)
