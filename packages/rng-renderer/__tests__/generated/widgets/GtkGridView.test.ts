...
describe("GridView", () => {
  ...
  test("should connect onNotifyMaxColumns", () => {
    ...
    const handler = widget.handlers["notify::maxColumns"]
    expect(handler).toBeDefined()
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::maxColumns", expect.any(Function))
  })
  ...
  test("should connect onNotifyMinColumns", () => {
    ...
    const handler = widget.handlers["notify::minColumns"]
    expect(handler).toBeDefined()
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::minColumns", expect.any(Function))
  })
