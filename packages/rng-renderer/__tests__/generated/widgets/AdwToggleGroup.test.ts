import { ToggleGroup } from "@/generated/widgets.ts"
import Gtk from "@/generated/girs/node-gtk-4.0.js"

describe("ToggleGroup", () => {
  let widget: ToggleGroup

  beforeEach(() => {
    widget = new ToggleGroup({}, ToggleGroup.createNode(    ))
  })
  ...
  test("should connect onNotifyActiveName", () => {
    ...
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::activeName", expect.any(Function))
  })
  ...
  test("should connect onNotifyNToggles", () => {
    ...
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::nToggles", expect.any(Function))
  })
  ...
  test("should connect onNotifyToggles", () => {
    ...
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::toggles", expect.any(Function))
  })
