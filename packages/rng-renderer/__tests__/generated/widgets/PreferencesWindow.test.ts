import { PreferencesWindow } from "@/generated/widgets.ts"
import Gtk from "@/generated/girs/node-gtk-4.0.js"

describe("PreferencesWindow", () => {
  let widget: PreferencesWindow

  beforeEach(() => {
    widget = new PreferencesWindow({}, PreferencesWindow.createNode(    ))
  })
  ...
  test("should connect onNotifySearchEnabled", () => {
    ...
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::searchEnabled", expect.any(Function))
  })
  ...
  test("should connect onNotifyVisiblePageName", () => {
    ...
    handler()
    expect(callback).toHaveBeenCalled()
    expect(widget.node.on).toHaveBeenCalledWith("notify::visiblePageName", expect.any(Function))
  })
