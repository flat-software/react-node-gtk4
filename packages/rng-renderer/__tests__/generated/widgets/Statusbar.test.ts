import { Statusbar } from "@/generated/widgets.ts"
...
describe("Statusbar", () => {
  ...
  test("should connect onTextPopped", () => {
    ...
    handler()
    expect(widget.node.on).toHaveBeenCalledWith("text-popped", expect.any(Function))
  ...
  test("should connect onTextPushed", () => {
    ...
    expect(handler).toBeDefined()
    handler()
    expect(widget.node.on).toHaveBeenCalledWith("text-pushed", expect.any(Function))
    expect(callback).toHaveBeenCalled()
  })
