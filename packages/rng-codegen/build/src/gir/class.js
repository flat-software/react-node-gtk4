...
export class GirClass extends GirElement {
    _typeDependencies;
    _props;
    _signals;
    _interfaces;
    get parent() {
        ...
        const parentName = last(this.data.parents.filter(p => p.type === "parent"))
            ?.qualifiedParentName || "";
    ...
    get isWidget() {
        ...
        const parents = Object.values(this.data.inherit).map(c => c.class.qualifiedName);
    ...
    get props() {
        ...
        ].filter(p => p.rawName !== "__gtype__"), p => p.name));
    }
    get signals() {
